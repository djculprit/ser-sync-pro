...
- **Feat: per-session file logging, volume-relative like the Java tool**: The Python app previously had no persistent logging at all — `main.py --cli` only configured a console handler, and the GUI never touched the `logging` module, so pipeline `logger.info()`/`logger.error()` calls went nowhere in GUI mode. New `python/core/session_log.py` adds a `session_log_file(name, serato_path)` context manager that attaches a `FileHandler` to the shared `cdd_sync` logger for the duration of a run. Log location mirrors the original Java tool exactly (`cdd_sync_main.java`: `seratoDir.getParentFile()` + `"cdd-sync-pro/logs"`) — `<parent of _Serato_>/cdd-sync-pro/logs/<name>-<timestamp>.log` — so logs live on the same external volume as the Serato library instead of collecting under wherever the app happens to be launched from.
  - `python/sync/pipeline.py`: `run_sync()` is now a thin wrapper opening a `"sync"` session log around the renamed `_run_sync()`. Every existing `logger.info`/`logger.error` call across `pipeline.py`, `backup.py`, `database_fixer.py`, `dupe_mover.py`, `media_library.py`, and `pref_sorter.py` is captured automatically since they all share the `cdd_sync` logger — no call-site changes needed.
  - `python/sync/session_fixer.py`: `scan_broken_paths()` and `fix_broken_paths()` wrap their internal `_scan_broken_paths_impl()` / `_fix_broken_paths_impl()` the same way (`"session-scan"` / `"session-fix"` logs). `fix_broken_paths()` calls the unwrapped scan internally so a fix run produces one log file, not two. Session fixer previously only supported a `log_callback` (GUI display) or bare `print`; a new `_make_log()` helper now also threads every message through `logging.getLogger("cdd_sync")` so it reaches the file the same way pipeline output does.
  - Simplified: unlike the Java version's seven per-step log files (main + dupe + path-fixes + step1-4), this is one consolidated log per run — every message is still timestamped and attributable by content.
